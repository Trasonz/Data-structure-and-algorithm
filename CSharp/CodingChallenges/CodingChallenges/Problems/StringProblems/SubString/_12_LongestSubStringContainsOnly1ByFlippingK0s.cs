...
internal class _12_LongestSubStringContainsOnly1ByFlippingK0s
{
    ...
    public void Run()
    {
        ...
        {
            var result = testCase.Key.FindLengthOfLongestSubStringContainsOnly1ByFlippingK0s(testCase.Value);
            Console.WriteLine($"The length of the longest sub-string that contains only '1's with {testCase.Value} flipping times of '0' is {result}");
        }
    }
}
